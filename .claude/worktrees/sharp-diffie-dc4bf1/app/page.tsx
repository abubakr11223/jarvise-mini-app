...
import { Mic, Grid, Menu, X, Bookmark, FileText, Send, User, Bot, ChevronRight,
  ...
  HandCoins, CheckSquare, Square, MessageCircle, Megaphone,
  Camera, StickyNote, CheckCircle2, Circle, Plus } from 'lucide-react'
...
export default function Home() {
  ...
  const addExpense = (name: string, amount: number, type: string) => {
    ...
    // Server ga ham saqlash (bot chat bilan sync uchun)
    fetch('/api/expenses', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ expenses: [newExp] }),
    }).catch(() => {})
  }
  ...
  const analyzeReceipt = async (file: File) => {
    if (!file) return
    setScanLoading(true)
    setMessages(p => [...p, { role: 'user', text: `📷 Chek rasmini yukladim` }])
    const ai_msg = { role: 'ai', text: '🔍 Chek tahlil qilinmoqda...' }
    setMessages(p => [...p, ai_msg])
    try {
      ...
      if (data.items?.length > 0) {
        const validItems = data.items.filter((i: { name: string; amount: number }) => i.amount >= 500)
        validItems.forEach((i: { name: string; amount: number }) => addExpense(i.name, i.amount, 'XARAJAT'))
        const lines = validItems.map((i: { name: string; amount: number }) => `💸 ${i.name} — ${i.amount.toLocaleString()} UZS`).join('\n')
        const total = data.total || validItems.reduce((s: number, i: { amount: number }) => s + i.amount, 0)
        setMessages(p => p.map((m, i) => i === p.length - 1 ? {
          role: 'ai',
          text: `🧾 **Chek tahlil qilindi!**\n\n${lines}\n\n💰 **Jami: ${total.toLocaleString()} UZS**\n\n✅ Moliyaga ${validItems.length} ta qo'shildi`,
        } : m))
        setTimeout(() => setExpOpen(true), 700)
      } else {
        setMessages(p => p.map((m, i) => i === p.length - 1 ? { role: 'ai', text: "❌ Chekdan ma'lumot ajratib bo'lmadi.\n\nAniqroq rasm oling yoki chekni to'g'ri yo'naltiring." } : m))
      }
    } catch {
      setMessages(p => p.map((m, i) => i === p.length - 1 ? { role: 'ai', text: '❌ Rasmni tahlil qilishda xato. Qayta urinib ko\'ring.' } : m))
  ...
  return (
    <main className="relative flex flex-col h-screen bg-[#0a0a0c] text-white font-sans overflow-hidden">
      ...
      <div className={`fixed inset-y-0 left-0 w-[280px] bg-[#0d0d10] z-50 transform transition-transform duration-300 flex flex-col border-r border-gray-800/60 shadow-2xl ${sidebar ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto py-3 px-2.5 flex flex-col gap-1">
          {[
            ...
            { icon: <StickyNote size={15} className="text-amber-400" />,       label: '📓 Eslatmalar (Notes)',    action: () => { setSidebar(false); setNotesOpen(true) } },
      ...
      <div className="absolute bottom-0 left-0 right-0 px-3 pb-4 pt-2 bg-gradient-to-t from-[#0a0a0c] via-[#0a0a0c]/95 to-transparent z-10">
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            className="w-[52px] h-[52px] shrink-0 rounded-full bg-[#1a1a1f] border border-gray-700/80 flex items-center justify-center active:scale-90 disabled:opacity-40 transition-all">
            {scanLoading ? (
              <span className="w-4 h-4 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" />
            ) : (
              <Camera size={19} className="text-amber-400" />
            )}
          </button>
          <input
            ref={receiptInputRef}
            ...
            accept="image/*"
            className="hidden"
            onChange={e => { const f = e.target.files?.[0]; if (f) { analyzeReceipt(f); e.target.value = '' } }}
          />
          {/* Input */}
      ...
      {debtOpen && (
        <div className="fixed inset-0 z-50 flex flex-col bg-[#0a0a0c] animate-slide-up">
          ...
          <div className="flex-1 overflow-y-auto px-4 pt-3 pb-4 flex flex-col gap-2">
            {debts.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-2 opacity-30 py-16"><HandCoins size={40} strokeWidth={1} /><p className="text-sm">Bo'sh</p><p className="text-xs text-center">Chatda "Rashidga 50 ming berdim" deb yozing</p></div>
            ) : (
              <>
                {/* Active debts */}
                {debts.filter(d => !d.paid).length > 0 && (
                  <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Faol qarzlar</p>
                )}
                {debts.filter(d => !d.paid).map(d => (
                  <div key={d.id} className="flex items-center gap-3 bg-[#111114] border border-gray-800/60 rounded-2xl px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs font-bold ${d.dir === 'gave' ? 'text-green-400' : 'text-red-400'}`}>
                        {d.dir === 'gave' ? '📤 Men berdim' : '📥 Men oldim'} — {d.amount.toLocaleString()} UZS
                      </p>
                    ...
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                      <button
                        onClick={() => setDebts(p => p.map(x => x.id === d.id ? { ...x, paid: true } : x))}
                        className="flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 bg-green-500/10 border border-green-500/20 rounded-xl text-green-400 active:scale-90">
                        <CheckCircle2 size={11} /> To'landi
                      </button>
                ...
                {debts.filter(d => d.paid).length > 0 && (
                  <>
                    <p className="text-[10px] text-gray-600 uppercase tracking-wider font-bold mt-2">To'langan</p>
                    {debts.filter(d => d.paid).map(d => (
                      <div key={d.id} className="flex items-center gap-3 bg-[#111114]/60 border border-gray-800/30 rounded-2xl px-4 py-3 opacity-50">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-gray-700/20 border border-gray-700/30 shrink-0">
                          <CheckCircle2 size={18} className="text-gray-500" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-400 line-through">{d.person}</p>
                          <p className="text-xs text-gray-600">{d.amount.toLocaleString()} UZS · {d.date}</p>
                        </div>
      ...
      {notesOpen && (
        <div className="fixed inset-0 z-50 flex flex-col bg-[#0a0a0c] animate-slide-up">
          <header className="flex justify-between items-center px-4 py-3.5 border-b border-gray-800 bg-[#111114] shrink-0">
            <h2 className="text-base font-bold flex items-center gap-2"><StickyNote size={16} className="text-amber-400" />Eslatmalar</h2>
            <button onClick={() => setNotesOpen(false)} className="p-2 bg-[#1a1a1f] rounded-full"><X size={16} /></button>
          </header>
          {/* Add note */}
          <div className="px-4 pt-3 shrink-0 space-y-2">
            <input
              value={noteInput.title}
              onChange={e => setNoteInput(p => ({ ...p, title: e.target.value }))}
              placeholder="Sarlavha..."
              className="w-full bg-[#1a1a1f] border border-gray-700 rounded-xl px-4 py-2.5 text-sm outline-none"
              style={{ fontSize: '16px' }}
            />
            <div className="flex gap-2">
              <textarea
                ...
                rows={2}
                className="flex-1 bg-[#1a1a1f] border border-gray-700 rounded-xl px-4 py-2.5 text-sm outline-none resize-none"
                style={{ fontSize: '16px' }}
              />
              <button
                onClick={() => {
                  if (!noteInput.content.trim()) return
                  setNotes(p => [{ id: Date.now(), title: noteInput.title || noteInput.content.slice(0, 30), content: noteInput.content, date: today() }, ...p])
                  setNoteInput({ title: '', content: '' })
                }}
                className="w-14 bg-amber-500 rounded-xl text-sm font-bold flex items-center justify-center active:scale-95">
                <Plus size={20} />
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 pt-3 pb-4 flex flex-col gap-2">
            {notes.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-2 opacity-30 py-16">
                <StickyNote size={40} strokeWidth={1} />
                <p className="text-sm">Hech qanday eslatma yo'q</p>
              </div>
            ) : notes.map(n => (
              <div key={n.id} className="bg-[#111114] border border-amber-500/20 rounded-2xl px-4 py-3">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => setNotes(p => p.filter(x => x.id !== n.id))} className="p-1 rounded-lg bg-[#1a1a1f] ml-1">
                ...
                <button
                  onClick={() => { setInputText(n.content.slice(0, 200)); setNotesOpen(false) }}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/80 backdrop-blur-md">
          <div className="w-full max-h-[78%] bg-[#111114] rounded-t-[28px] p-5 relative border-t border-gray-800 animate-slide-up shadow-2xl overflow-y-auto">
            ...
            {[
              ...
              {
                title: '📓 Shaxsiy', apps: [
                  { icon: <StickyNote size={20} className="text-amber-400" />, label: 'Eslatmalar', url: '__notes__', bg: 'bg-amber-500/15 border-amber-500/25' },
                ]
              },
            ].map(sec => (
              <div key={sec.title} className="mb-5">
                ...
                <div className="grid grid-cols-4 gap-3">
                  {sec.apps.map(app => (
                    <button key={app.label} onClick={() => { setAppsOpen(false); if (app.url === '__notes__') { setNotesOpen(true) } else { openApp(app.url) } }} className="flex flex-col items-center gap-2 active:scale-90 transition-transform">
